...
object Server extends ServerModuleInterface {
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var nextSeqNr: Int = 1
      var events: Map[Int, Event] = Map.empty
      (event: Event) => {
        events += (event.sequenceNr -> event)
        var result = Seq[Event]()
        while (events.contains(nextSeqNr)) {
          result = result :+ events(nextSeqNr)
          nextSeqNr += 1
        }

        events = events.removedAll(result.map(_.sequenceNr))
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = {
    ...
    val (event, vFollowers) = eventAndFollowers
    event match {

      case followers.model.Event.Follow(sequenceNr, fromUserId, toUserId) =>
        vFollowers.get(fromUserId).exists(_.contains(userId))

      case followers.model.Event.Unfollow(sequenceNr, fromUserId, toUserId) =>
        vFollowers.get(fromUserId).exists(_.contains(userId))

      case followers.model.Event.Broadcast(sequenceNr) =>
        true

      case followers.model.Event.PrivateMsg(sequenceNr, fromUserId, toUserId) =>
        vFollowers.get(fromUserId).exists(_.contains(userId))

      case followers.model.Event.StatusUpdate(sequenceNr, fromUserId) =>
        vFollowers.get(userId).exists(_.contains(fromUserId))
    }
  }
...
class Server()(implicit executionContext: ExecutionContext, materializer: Materializer)
  ...
  /**
    ...
    */
  val eventsFlow: Flow[ByteString, Nothing, NotUsed] = Flow.fromSinkAndSourceCoupled(inboundSink, Source.maybe[Nothing])
  ...
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut
      .filter { case (event, vFollowers) =>
        event match {
          case followers.model.Event.Follow(sequenceNr, fromUserId, toUserId) => userId == toUserId
          case followers.model.Event.Unfollow(sequenceNr, fromUserId, toUserId) => false
          case followers.model.Event.Broadcast(sequenceNr) => true
          case followers.model.Event.PrivateMsg(sequenceNr, fromUserId, toUserId) => userId == toUserId
          case followers.model.Event.StatusUpdate(sequenceNr, fromUserId) =>
            vFollowers(userId).contains(fromUserId)
        }
      }
      .map { case (event, followers) => event.render }
  ...
  def clientFlow(): Flow[ByteString, ByteString, NotUsed] = {
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink
        .mapMaterializedValue(_.onComplete(clientIdPromise.complete))
