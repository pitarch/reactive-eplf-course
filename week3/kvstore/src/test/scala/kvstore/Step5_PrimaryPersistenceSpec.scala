...
trait Step5_PrimaryPersistenceSpec { this: KVStoreSuite =>
  ...
  @Test def `Step5-case2: Primary retries persistence every 100 milliseconds`(): Unit = {
    val arbiter = TestProbe()
    val persistence = TestProbe()
  ...
  @Test def `Step5-case3: Primary generates failure after 1 second if persistence fails`(): Unit = {
    val arbiter = TestProbe()
    val persistence = TestProbe()
