...
object Async extends AsyncInterface {
  ...
  /**
    ...
    */
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = eventuallyX map {
    _ % 2 == 0
  }
  ...
  def sequenceComputations[A, B](
    ...
    fut1.flatMap { a => makeAsyncComputation2().map { b => (a, b) } }
    //    for {
    ...
    //    f1.onComplete {
    //      case Success(a) => makeAsyncComputation2().onComplete {
    //      case Failure(exception) => promise complete Failure(exception)
    //        case Success(b) => promise complete Success(a, b)
    //        case Failure(exception) => promise complete Failure(exception)
    //      }
    //    }
    //    promise.future
  ...
  def concurrentComputations[A, B](
    ...
    val aFut = makeAsyncComputation1()
    val bFut = makeAsyncComputation2()
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    makeAsyncComputation() transformWith {
      ...
      case Failure(exception) => if (maxAttempts > 1) insist(makeAsyncComputation, maxAttempts - 1) else Future.failed(exception)
    }
  }
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = {
    ...
    () => {
      ...
      def continuation(result: Try[Int]): Unit = {
